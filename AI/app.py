...
with gr.Blocks(title="OLM North Star") as demo:
    ...
    msg.submit(stream_chat, [msg, chatbot], [chatbot], api_name="chat").then(
        lambda: "", None, [msg]
    )
    
    clear.click(clear_chat, None, [chatbot], api_name="clear_chat")
    
    # New API for saving blogs
    blogs_input = gr.Textbox(visible=False)
    blogs_output = gr.Textbox(visible=False)
    save_btn = gr.Button("Save Blogs", visible=False)
    save_btn.click(save_blogs, [blogs_input], [blogs_output], api_name="save_blogs")
